...
function BatchDetails(props) {
  ...
  const handleClick = async (e) => {
    ...
    const response = await fetch(
      "http://localhost:8000/faculty/update-marks/",
      {
        method: "POST",
        headers: {
  ...
  const handlechg = (e) => {
    ...
    let b = obj.Analysis;
    let c = obj.Answers;
    let d = obj.Originality;
    let f = obj.Quality;
    if (e.target.name === "Identification") a = e.target.value;
    if (e.target.name === "Analysis") b = e.target.value;
    if (e.target.name === "Answers") c = e.target.value;
    if (e.target.name === "Originality") d = e.target.value;
    if (e.target.name === "Quality") f = e.target.value;
    if (a === "" || a === -1) a = 0;
    if (b === "" || b === -1) b = 0;
    if (c === "" || c === -1) c = 0;
    if (d === "" || d === -1) d = 0;
  ...
  const setmarks = (index, usn) => {
    ...
    setobj({
      ...obj,
      Identification: stdent_ph1[index].Identification,
      Analysis: stdent_ph1[index].Analysis,
      Originality: stdent_ph1[index].Originality,
      Quality: stdent_ph1[index].Quality,
      Answers: stdent_ph1[index].Answers,
      Total: stdent_ph1[index].Total,
    });

    settotal(
      parseInt(stdent_ph1[index].Total !== -1 ? stdent_ph1[index].Total : 0)
    );
  };
  ...
  return (
    <>
        ...
        <div className="modal-dialog">
          <div className="modal-content">
            ...
            <div className="modal-body" style={{ background: "burlywood" }}>
              <form
                className="question  p-3 border-bottom"
              ...
              >
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    ...
                    <input
                      value={
                        obj.Identification !== -1 ? obj.Identification : " "
                      }
                      type="number"
                      className="mx-2"
                      placeholder="Enter No"
                      onChange={(e) => handlechg(e, 10)}
                      name="Identification"
                      required
                    ></input>
                  </label>
                ...
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    {tablePhase == 1 ? (
                      ...
                      <>Effective usage of modern tools : </>
                    ) : (
                      <>Work effectively as a team member/leader : </>
                    )}
                    (10 Marks)
                    <input
                      value={obj.Analysis !== -1 ? obj.Analysis : " "}
                      type="number"
                      className="mx-2"
                      onChange={(e) => handlechg(e, 10)}
                      placeholder="Enter No"
                ...
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    {tablePhase == 1 ? (
                      <>Originality of problem statement : </>
                    ) : tablePhase == 2 ? (
                      <>Work effectively as a team member/leader : </>
                    ) : (
                      <>Presentation, report writing and submission</>
                    )}
                    (10 Marks)
                    <input
                      value={obj.Originality !== -1 ? obj.Originality : " "}
                      type="number"
                      name="Originality"
                      onChange={(e) => handlechg(e, 10)}
                      className="mx-2"
                ...
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    ...
                    (10 Marks)
                    <input
                      value={obj.Quality !== -1 ? obj.Quality : " "}
                      type="number"
                      onChange={(e) => handlechg(e, 10)}
                      name="Quality"
                      className="mx-2"
                ...
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    ...
                    (10 Marks)
                    <input
                      value={obj.Answers !== -1 ? obj.Answers : " "}
                      type="number"
                      onChange={(e) => handlechg(e, 10)}
                      name="Answers"
                      className="mx-2"
                ...
                <div className="ans ml-2  my-3">
                  <label className="radio">
                    Total Phase {tablePhase}(50 Marks)
            ...
            >
              ...
              <button
                disabled={
                  obj.Identification.length === 0 ||
                  obj.Analysis.length === 0 ||
                  obj.Originality.length === 0 ||
                  obj.Quality.length === 0 ||
      ...
      <div
        id="batch-details-student"
        className="login-box my-5"
          ...
          <table className="fl-table my-5">
            <thead>
              <tr>
                ...
                <th>Name</th>
                <th>
                  Identification and formulation of problem statement(10 marks)
                </th>
